> ## ⚠️ 结论更正（2026-09-19）
>
> 本清单成文于"接线错误 + 内核 CMA 回归 + 镜像 NCCL 残留"阶段，其中若干条目（尤其第 0 节的三角拓扑对照）
> **已被推翻**。修正后 RoCE 可用：单流 **28.5–35.4 tok/s**、4 并发 **67.5–75.8 tok/s**。
> 当前权威结论见 **README §1–§4**；内核/驱动必须为 `6.17.0-1031-nvidia` + `580.173.02`。
> 清单中与传输无关的条目（统一内存预算、Engram、page cache、SPS 等）仍然有效。
...
| 社区记录的坑                                                                                   | 你的现场状态                                                                                                                 |
| ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **~~CX7「三角」实际只有 3 根线；NCCL 按索引对齐 channel→NIC，在 3 节点 2 口拓扑上无解~~** ⚠️ **2026-09-19 更正：该结论已被推翻** —— 把接线改成"每条缆 p0↔p1"（交叉环）后索引对齐不再无解，配 `SUBNET_AWARE_ROUTING=1` 一次通过（README §1）                              | ✅ **你已独立实测闭环**（`search.cc:609`，index-0 = 最小 PCI = 三台都是 p0，穷举 0 解）。社区同名结论见 LLMKube「Three-Spark ring」                    |
